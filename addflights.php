...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $checkQuery = "SELECT * FROM flight_routes 
                   WHERE airlineProvide = ? 
                     AND routeDeparturePoint = ? 
                     AND routeArrivalPoint = ? 
                     AND seatType = ?";
    $checkStmt = $conn->prepare($checkQuery);
    $checkStmt->bind_param("ssss", $airline, $departure, $arrival, $seatType);
    $checkStmt->execute();
    $checkResult = $checkStmt->get_result();

    if ($checkResult->num_rows > 0) {
        echo "<script>alert('Flight already registered. Flight was not added.'); window.location.href='adminflights.php';</script>";
    } else {
        ...
        if ($stmt->execute()) {
            echo "<script>alert('Flight added successfully!'); window.location.href='adminflights.php';</script>";
        } else {
